...
def fspecial(type, radius):
    if type == 'average':
        return np.ones((radius, radius), dtype=float) / (radius * radius)
    elif type == 'disk':
        ...
        m1 = (radius ** 2 < (maxxy + 0.5) ** 2 + (minxy - 0.5) ** 2) * (minxy - 0.5) + \
             (radius ** 2 >= (maxxy + 0.5) ** 2 + (minxy - 0.5) ** 2) * np.sqrt(radius ** 2 - (maxxy + 0.5) ** 2, dtype=np.complex128)
        ...
        if (crad > 0) and (radius > crad - 0.5) and (radius ** 2 < (crad - 0.5) ** 2 + 0.25):
            m1 = np.sqrt(radius ** 2 - (crad - 0.5) ** 2, dtype=np.complex128)
            m1n = m1 / radius
